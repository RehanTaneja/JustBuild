...
class OrchestratorAgent:
    ...
    def __init__(self, product_idea: str, output_root: Path, max_retries: int = 2) -> None:
        request = BuildRequest(product_idea=product_idea, output_root=output_root)
        self.context = BuildContext(request=request)
        self.logger = BuildLogger(self.context)
        deps = AgentDependencies(context=self.context, logger=self.logger)
    ...
    def run(self) -> BuildContext:
        with self.logger.timed(self.name, "Initialized orchestration workflow", "orchestration", 0):
        ...
        with self.logger.timed(self.name, "Produced architecture plan", "orchestration", 1):
        ...
        planning_feedback = self._detect_architecture_issues()
        ...
        for attempt in range(1, self.max_retries + 2):
            ...
            with self.logger.timed(self.name, f"Implementation pass {attempt}", "orchestration", attempt):
            ...
            with self.logger.timed(self.name, f"Testing pass {attempt}", "orchestration", attempt):
                ...
                failure_reports = test_result.failure_reports
        ...
        with self.logger.timed(self.name, "Evaluation completed", "orchestration", len(self.context.iterations)):
        ...
        with self.logger.timed(self.name, "Persisted build summary", "observability", len(self.context.iterations)):
