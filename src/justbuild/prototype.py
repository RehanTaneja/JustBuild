...
from .models import ArchitecturePlan, ImplementationPlan, ImplementationPlanFile, ProductSpecification
...
def default_static_web_plan(spec: ProductSpecification) -> ImplementationPlan:
    return ImplementationPlan(
        ...
        files=[
            ImplementationPlanFile(
                path="index.html",
                purpose=f"Primary HTML entrypoint for {spec.title}.",
                required=True,
            ),
            ImplementationPlanFile(
                path="styles.css",
                purpose="Shared stylesheet for the browser prototype.",
                required=True,
            ),
            ImplementationPlanFile(
                path="app.js",
                purpose="Client-side interaction logic for generated responses.",
                required=True,
                depends_on=["index.html"],
            ),
            ImplementationPlanFile(
                path="README.md",
                purpose="How to open and use the prototype.",
                required=True,
            ),
        ],
        notes=["Defaulted to the static_web prototype profile."],
    )


def default_static_web_file_content(
    file_path: str,
    spec: ProductSpecification,
    architecture: ArchitecturePlan,
) -> str | None:
    if file_path == "index.html":
        return render_index_html(spec, architecture)
    if file_path == "styles.css":
        return render_styles_css()
    if file_path == "app.js":
        return render_app_js(spec)
